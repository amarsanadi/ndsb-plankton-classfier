...
dofile('settings.lua')
lexi_version = "V6cudnn" 

--model parameters

useCuda = true
if useCuda then
  require 'cudnn' --switching to cunn
  require 'cutorch'
...
if loadsavedModel==true then 
  ...
  CNN:add(cudnn.SpatialConvolution(1,128,8,8,1,1)) 
  CNN:add(cudnn.ReLU())
  CNN:add(cudnn.SpatialMaxPooling(2,2,2,2))

  CNN:add(cudnn.SpatialConvolution(128,256,6,6,1,1)) 
  CNN:add(cudnn.ReLU())

  CNN:add(cudnn.SpatialConvolution(256,256,5,5,1,1)) 
  CNN:add(cudnn.ReLU())
  CNN:add(cudnn.SpatialMaxPooling(2,2,2,2))

  CNN:add(cudnn.SpatialConvolution(256,384,5,5,1,1)) 
  CNN:add(cudnn.ReLU())

  CNN:add(cudnn.SpatialConvolution(384,384,4,4,1,1)) 
  CNN:add(cudnn.ReLU())
  CNN:add(cudnn.SpatialMaxPooling(2,2,1,1))
--[[
